  ...
  <div class="admin-page container">
    <header class="page-header surface-card anim-fade-up">
      <div>
        ...
        <p class="page-subtitle">查看和管理平台所有车位，支持搜索、筛选和状态操作。</p>
      </div>
    </header>
    ...
    <div class="toolbar surface-card anim-fade-up anim-d1">
      <div class="toolbar-fields">
        ...
        />
        <select v-model="productFilter" class="form-control" @change="resetAndLoad">
          <option value="">全部产品</option>
          <option value="chatgpt-plus">Plus</option>
          <option value="chatgpt-team">Business</option>
          <option value="chatgpt-pro">Pro</option>
        </select>
    ...
    <div v-if="loading && rides.length === 0" class="loading-container surface-card anim-fade-up anim-d1">
      <div class="spinner"></div>
      <p>加载车位数据中</p>
    </div>

    <div v-else-if="rides.length === 0" class="empty-state surface-card anim-fade-up anim-d1">
      <ParkingSquare :size="38" class="empty-icon" />
      <h3>暂无车位数据</h3>
      <p>没有找到匹配的车位，请调整搜索条件。</p>
    </div>

    <template v-else>
      <div class="summary-bar anim-fade-up anim-d2">
        <div class="summary-item">
          <span class="summary-label">总车位</span>
        ...
        <div class="summary-item">
          ...
          <strong class="summary-value">{{ rides.length }} 条</strong>
        </div>
      </div>
      ...
      <div class="table-container surface-card anim-fade-up anim-d3">
        <table class="records-table">
          ...
          <tbody>
            <tr v-for="(ride, idx) in rides" :key="ride.id" class="table-row-animate" :class="ride.product" :style="{ animationDelay: (idx * 0.03) + 's' }">
            <td>
              <router-link :to="`/ride/${ride.id}`" class="record-title">{{ ride.title }}</router-link>
            </td>
            <td>
            ...
            <td class="price-text">¥{{ Number(ride.price_per_month).toFixed(0) }}</td>
            ...
            <td>{{ ride.owner_nickname || '-' }}</td>
            <td class="date-col">{{ formatDate(ride.created_at) }}</td>
            <td class="text-right">
              <div class="action-btns">
                ...
                >
                  {{ ride.status === 'open' ? '关闭' : '重开' }}
                </button>
                <button
                  v-if="ride.purchase_count === 0"
                  type="button"
...
import { Search, ParkingSquare } from '@lucide/vue'
import { adminApi } from '../../api/admin'
import type { AdminRideListItem } from '../../types'

const PAGE_SIZE = 20

const loading = ref(false)
const rides = ref<AdminRideListItem[]>([])
const total = ref(0)
...
const loadRides = async () => {
  ...
  try {
    ...
    rides.value = res.data.items
    total.value = res.data.total
  } catch {
    message.value = '加载车位数据失败'
  } finally {
    loading.value = false
  }
}
...
const toggleRideStatus = async (ride: AdminRideListItem) => {
  const newStatus = ride.status === 'open' ? 'closed' : 'open'
  const action = newStatus === 'closed' ? '关闭' : '重开'
  if (!confirm(`确定要${action}车位「${ride.title}」吗？`)) return

  try {
    await adminApi.updateRideStatus(ride.id, newStatus)
    ride.status = newStatus
    message.value = `已${action}车位「${ride.title}」`
  } catch {
    message.value = `${action}失败，请重试`
  }
}

const deleteRide = async (ride: AdminRideListItem) => {
  if (!confirm(`确定要删除车位「${ride.title}」吗？此操作不可撤销。`)) return
